...
class Sklearn_HoldOut(Strategy):

    def __init__(self, params):
        'Constructor'
        super(Sklearn_HoldOut, self).__init__('Sklearn_HoldOut', params)
    ...
    def fit(self, model, data, new_data):
        ...
        if params['problem_type'].lower() == 'classification':
            total_metrics_names = ["acc", "tn", "fp", "fn", "tp", "recall",
                             "specificity", "precision", "f1", "Negative predictive value", "False positive rate", "False negative rate", "False discovery rate"]
