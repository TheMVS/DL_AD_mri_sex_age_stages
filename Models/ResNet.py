...
class ResNet(Model):
    ...
    def __init__(self, params):
        'Constructor'
        super(ResNet, self).__init__('ResNet', params)
    ...
    def get_architecture(self, params):
        ...
        initial_model = resnet50.ResNet50()  # TODO: tal vez incluir image_data_format='channels_last'

        for i in range(len(initial_model.layers) - 1 - params['last_layers']):
            initial_model.layers[i].trainable = False

        model = initial_model.layers[len(initial_model.layers) - 1 - params['last_layers']].output  # assuming you want the 3rd layer from the last
